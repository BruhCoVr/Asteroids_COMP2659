...
;-----------------------------------------------------------------------------
_get_video_base:
    move.l  #$FFFF8201,a0     ; Point to video base high byte register (use $ instead of 0x)
    moveq   #0,d0             ; Clear all 32 bits of return register (alternative to clr.l)
...
_set_video_base:
    ...
    move.l  #$FFFF8201,a0     ; Point to video base register pair (use $ instead of 0x)
    movep.w d0,0(a0)          ; ATOMIC write to both 8201 (high) and 8203 (middle)
